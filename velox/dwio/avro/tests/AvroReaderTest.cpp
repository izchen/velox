  ...
  std::shared_ptr<exec::test::TempFilePath> writeComplexNestedRecord() const {
    ...
    auto filePath = writeAvroFile(
        schemaJson, [](auto& writer, const ::avro::ValidSchema& schema) {
          ...
          auto& metaRecord =
              record.fieldAt(0).value<::avro::GenericRecord>();
          auto& ids = metaRecord.fieldAt(0)
          ...
          auto& attrs = metaRecord.fieldAt(1)
                            .value<::avro::GenericMap>()
                            .value();
          attrs.clear();
          auto attrsSchema = schema.root()->leafAt(0)->leafAt(1);
          auto attrValueSchema = attrsSchema->leafAt(1);
          ...
          auto& flags = payloadRecord.fieldAt(0)
                            .value<::avro::GenericArray>()
                            .value();
          flags.clear();
          ...
          auto& properties = payloadRecord.fieldAt(1)
                                 .value<::avro::GenericMap>()
                                 .value();
          properties.clear();
          ...
          auto& propertyRecord =
              propertyDatum.value<::avro::GenericRecord>();
          propertyRecord.fieldAt(0).value<std::string>() = "p1";
          auto& values = propertyRecord.fieldAt(1)
                             .value<::avro::GenericArray>()
                             .value();
          values.clear();
          values.emplace_back(static_cast<int32_t>(1));
          values.emplace_back(static_cast<int32_t>(2));
          properties.emplace_back("k1", propertyDatum);

          payloads.emplace_back(payloadDatum);
          writer.write(datum);
        });
    return filePath;
  }
