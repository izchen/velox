...
TEST_F(AvroReaderTest, unionMapping) {
  ...
  ASSERT_EQ(rowType->size(), 4);
  EXPECT_EQ(rowType->childAt(0)->kind(), TypeKind::INTEGER);
  EXPECT_EQ(rowType->childAt(1)->kind(), TypeKind::BIGINT);
  EXPECT_EQ(rowType->childAt(2)->kind(), TypeKind::DOUBLE);
  ASSERT_EQ(rowType->childAt(3)->kind(), TypeKind::ROW);

  const auto& mixedRow = rowType->childAt(3)->asRow();
...
TEST_F(AvroReaderTest, duplicateFieldNamesRejected) {
  const std::string schemaJson = R"JSON(
    {
      "type": "record",
      "name": "DupRecord",
...
TEST_F(AvroReaderTest, rejectsUnsupportedUnion) {
  const std::string schemaJson = R"JSON(
    {
      "type": "record",
      "name": "NullUnionRecord",
      "fields": [
...
TEST_F(AvroReaderTest, usesSchemaOverride) {
  ...
  EXPECT_EQ(rowType->nameOf(1), "b");
}

TEST_F(AvroReaderTest, invalidSchemaOverrideFails) {
  const std::string schemaJson = R"JSON(
    {
      "type": "record",
  ...
  auto filePath = writeAvroFile(
      ...
      [](auto& writer, const ::avro::ValidSchema& schema) {
        ::avro::GenericDatum datum(schema.root());
        datum.value<::avro::GenericRecord>().fieldAt(0).value<int32_t>() = 11;
        writer.write(datum);
      });
