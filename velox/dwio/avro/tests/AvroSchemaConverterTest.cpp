...
// Tests Avro schema conversion directly from avro-cpp schemas.
class AvroSchemaConverterTest : public AvroReaderTestBase {
 protected:
  RowTypePtr convertSchema(
    ...
    return std::static_pointer_cast<const RowType>(typeInfo->veloxType);
  }

  RowTypePtr convertSchema(
      const std::shared_ptr<TempFilePath>& filePath) const {
    return convertSchema(filePath, dwio::common::ReaderOptions{pool()});
  }
};

TEST_F(AvroSchemaConverterTest, allTypesSchemaMapping) {
  const auto filePath = writeAllTypesRecord();
  auto rowType = convertSchema(filePath);
...
TEST_F(AvroSchemaConverterTest, lowerCaseFieldNames) {
  ...
  auto rowType = convertSchema(filePath, options);
...
TEST_F(AvroSchemaConverterTest, rejectsDuplicateFieldNames) {
  ...
  dwio::common::ReaderOptions options{pool()};
  options.setFileColumnNamesReadAsLowerCase(true);
  EXPECT_THROW(convertSchema(filePath, options), VeloxRuntimeError);
}
...
TEST_F(AvroSchemaConverterTest, rejectsUnsupportedLogicalType) {
  ...
  EXPECT_THROW(convertSchema(filePath), VeloxUserError);
}
...
TEST_F(AvroSchemaConverterTest, rejectsUnsupportedDecimalMetadata) {
  const auto expectUnsupported = [&](const std::string& schemaJson,
    ...
    VELOX_ASSERT_THROW_CODE(
        convertSchema(filePath), error_code::kUnsupported, expectedError);
  };
