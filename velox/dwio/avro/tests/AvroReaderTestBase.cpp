...
std::shared_ptr<TempFilePath> AvroReaderTestBase::writeAllTypesRecord() const {
...
std::shared_ptr<TempFilePath> AvroReaderTestBase::writeComplexNestedRecord()
    const {
  const std::string schemaJson = R"JSON(
  {
    "type": "record",
  ...
  auto filePath = writeAvroFile(
      schemaJson, [](auto& writer, const ::avro::ValidSchema& schema) {
        ...
        auto& ids = metaRecord.fieldAt(0).value<::avro::GenericArray>().value();
        ids.emplace_back(static_cast<int64_t>(101));
        ids.emplace_back(static_cast<int64_t>(202));
        auto& attrs = metaRecord.fieldAt(1).value<::avro::GenericMap>().value();
...
std::shared_ptr<TempFilePath> AvroReaderTestBase::writeUnionRecord() const {
  const std::string schemaJson = R"JSON(
  {
    "type": "record",
    "name": "UnionRecord",
  ...
  auto filePath = writeAvroFile(
      schemaJson, [](auto& writer, const ::avro::ValidSchema& schema) {
        ::avro::GenericDatum datum(schema.root());
        auto& record = datum.value<::avro::GenericRecord>();
        auto writeRow = [&](std::string simpleString,
          ...
          simpleNestedRecord.fieldAt(0).value<int32_t>() = simpleNestedInt;
...
std::shared_ptr<TempFilePath> AvroReaderTestBase::writeLogicalUnionRecord()
    const {
  const std::string schemaJson = R"JSON(
  {
    "type": "record",
