...
uint64_t loadAvroScanBatchBytes(const ReaderOptions& options) {
  const auto& serdeParameters = options.serDeOptions().parameters;
  const auto it =
      serdeParameters.find(std::string(kAvroScanBatchBytesKey));
...
std::shared_ptr<AvroTypeInfo> buildTypeInfo(const ::avro::NodePtr& node, const ReaderOptions& options);
...
std::shared_ptr<AvroTypeInfo> buildUnionType(const ::avro::NodePtr& node, const ReaderOptions& options) {
  ...
  VELOX_CHECK(!nonNullInfos.empty(), "Unsupported Avro union without non-null branch.");
...
std::optional<ResolvedDatum> resolveDatum(
  ...
  if (datum.type() == ::avro::Type::AVRO_NULL) {
    VELOX_CHECK(
        info.nullable,
        "Encountered null value for non-nullable Avro schema.");
    return std::nullopt;
  }
...
void writeDatum(const ResolvedDatum& resolved, GenericWriter& writer) {
    ...
    switch (resolvedInfo.unionKind) {
      ...
      case AvroUnionKind::kStruct: {
        ...
        auto& childWriter = rowWriter.get_writer_at(
            static_cast<int32_t>(selectedIndex));
  ...
  switch (resolvedDatum.type()) {
    ...
    case ::avro::Type::AVRO_ARRAY: {
      ...
      const auto& elements = resolvedDatum.value<::avro::GenericArray>().value();
      arrayWriter.reserve(static_cast<vector_size_t>(elements.size()));
      for (const auto& element : elements) {
        auto resolvedElement = resolveDatum(*resolvedInfo.children.front(), element);
    ...
    case ::avro::Type::AVRO_MAP: {
      ...
      for (const auto& [key, valueDatum] : entries) {
        auto resolvedValue = resolveDatum(*resolvedInfo.children.front(), valueDatum);
    ...
    case ::avro::Type::AVRO_RECORD: {
      ...
      for (size_t i = 0; i < resolvedInfo.children.size(); ++i) {
        auto resolvedField = resolveDatum(*resolvedInfo.children[i], record.fieldAt(i));
...
struct AvroFileContents {
  ...
  std::unique_ptr<::avro::DataFileReader<::avro::GenericDatum>> makeReader() const {
  ...
  std::unique_ptr<::avro::DataFileReader<::avro::GenericDatum>> acquireReader() {
    if (cachedReader) {
...
AvroReader::AvroReader(
  ...
  contents_ = std::make_shared<AvroFileContents>(
      std::move(input), options, std::move(schema), std::move(typeInfo),
      std::move(reader), avroScanBatchBytes);
}
...
AvroRowReader::AvroRowReader(
  ...
  if (options.limit() >= static_cast<uint64_t>(std::numeric_limits<int64_t>::max())) {
...
int64_t AvroRowReader::nextReadSize(const uint64_t size) {
  ...
  const auto rowsByBytes = std::max<uint64_t>(
    1, contents_->avroScanBatchBytes / rowSize.value());
