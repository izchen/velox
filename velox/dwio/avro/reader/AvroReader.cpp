...
std::shared_ptr<AvroTypeInfo> buildReadRowTypeInfo(
  ...
  // Translates a ScanSpec child into read schema; constants are projected later.
...
std::shared_ptr<AvroTypeInfo> buildReadTypeInfo(
    ...
    const common::ScanSpec* scanSpec,
    const std::string& path) {
  // This path is already selected by its parent; sibling field pruning happens
  // by not calling this function for unselected row children.
  VELOX_DCHECK(requestedOutputType || scanSpec);
  ...
  switch (source.avroType) {
    case ::avro::Type::AVRO_ARRAY: {
      ...
      if (!requestedElementOutputType && !elementSpec) {
        ...
        // The ScanSpec selected the array as a whole without element-level
        // constraints. Keep the file element schema intact.
        return target;
      }
    ...
    case ::avro::Type::AVRO_MAP: {
      ...
      if (!requestedValueOutputType && !valueSpec) {
        VELOX_DCHECK(scanSpec);
        // The ScanSpec selected the map as a whole without value-level
        // constraints. Keep the file value schema intact.
        return target;
      }
  ...
  if (resolved.unionChildIndex.has_value()) {
    switch (resolvedInfo.unionKind) {
      ...
      case AvroUnionKind::kStruct: {
        ...
        auto& childWriter =
            rowWriter.get_writer_at(static_cast<int32_t>(selectedIndex.value()));
...
struct NestedEntrySelection {
  ...
  // True only when filtering or max-entry limit changes the entry set.
  // Offset compaction alone does not require dictionary wrapping.
...
VectorPtr projectArrayVector(
  ...
  if (hasElementFilter && elements->size() > 0) {
    ...
    elementSpec->applyFilter(
        *elements, elements->size(), passedEntries.data());
  }

  auto projectedElements = elementSpec
      ? projectComplexVector(elements, *elementSpec)
      : elements;
...
//   requestedType, and let projectAvroColumns() produce the final output.
