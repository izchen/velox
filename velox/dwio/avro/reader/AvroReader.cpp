...
///   map<string, V>              -> MAP(VARCHAR(), veloxType(V))  (Avro spec mandates string keys)
...
///   - other multi-branch unions -> ROW(member0..memberN, veloxType(branch0)..veloxType(branchN)),
...
///   - Record field names can be lower-cased via options.fileColumnNamesReadAsLowerCase(),
///     duplicated field names are rejected.
///   - Unsupported type fall through to VELOX_UNSUPPORTED().
...
std::shared_ptr<AvroTypeInfo> buildTypeInfo(
  ...
  if (logicalType != ::avro::LogicalType::Type::NONE && info->logicalType == AvroLogicalType::kNone) {
    VELOX_UNSUPPORTED(
...
std::optional<ResolvedDatum> resolveUnionAndNull(
    const AvroTypeInfo& info,
    const ::avro::GenericDatum& datum) {
  // For unions, datum.type() reflects the active branch, matching both AVRO_NULL
  // values and unions with NULL as the selected branch.
  ...
  // In AvroTypeInfo, the AVRO_NULL union branch represents nullability rather
  // than a child type, so we remap the union branch index accordingly.
  ...
  switch (info.unionKind) {
    case AvroUnionKind::kSimple:
      return ResolvedDatum{&*info.children[childIndex], &datum, 0};
  ...
  VELOX_UNREACHABLE("Encountered union datum without union schema.");
}
...
void writeDatum(const ResolvedDatum& resolved, GenericWriter& writer) {
  ...
  switch (resolvedDatum.type()) {
    ...
    default:
      // Unreachable: unsupported Avro types must be rejected during
      // avro-to-velox schema resolution.
      VELOX_UNREACHABLE(
...
AvroReader::AvroReader(
  ...
  std::shared_ptr<const TypeWithId> schemaWithId;
  if (scanSpec) {
    schemaWithId = TypeWithId::create(rowType, *scanSpec);
  } else {
...
AvroRowReader::AvroRowReader(
    ...
    : contents_(std::move(contents)),
      ...
      splitLimit_(options.limit() >= static_cast<uint64_t>(kMaxSafeAvroReaderPosition)
